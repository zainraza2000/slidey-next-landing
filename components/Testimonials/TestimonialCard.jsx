...
const TestimonialCard = ({ item, isBlur,style }) => {
  return (
    <div
      key={item.id}
      className={`2xl:w-[486px] 2xl:h-[550px] xl:w-[420px] xl:h-[500px]  md:w-[33%] md:h-[350px] mx-auto  2xl:gap-8 flex flex-col justify-between  bg-[#fff] testinomial-boxShadow md:rounded-3xl rounded-md
    ...
    >
      ...
      <div className=" flex md:gap-5 gap-1 items-center">
        ...
        <div className=" flex flex-col justify-center xl:gap-2 md:gap-[6px]">
          <h3 className=" fontGivonic xl:text-3xl md:text-xl text-[9px] font-semibold xl:leading-tight md:leading-tight">
            {item.firstname} <br /> {item.lastname}
          </h3>
          <p className="  fontGivonic xl:text-lg md:text-sm text-[6px] text-[#4C4C4C] font-semibold">
            {item.designation}
          </p>
        </div>
      </div>
      <p className="fontGivonic xl:text-xl md:text-sm text-[6px] font-semibold">{item.message}</p>
      <div className=" flex justify-between items-center">
        <div className=" flex md:gap-3 gap-1 items-center">
          <p className=" text-[#4D4D4D] xl:text-base md:text-sm text-[6px]">Read More</p>
          <Image src={arrow} width={10} height={10} className=" md:w-auto md:h-auto w-[6px] h-[6px]" />
        </div>
        <Image
